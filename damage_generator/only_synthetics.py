...
def create_random_mask(target_size, df, df_synthetic, df_counts,
                       use_synthetic=True,      # Force True by default
                       min_artifacts=5, max_artifacts=100, 
                       rescale=True, 
    ...
    # ============================
    # ONLY SYNTHETIC VERSION
    # ============================
    if not uniform_sample:

        # Use synthetic counts instead of real ones (safer)
        num_dust = sample_num_artifacts(df_counts, 'dust', target_size)      # you can keep this for now
        ...
        if verbose:
            print(f"Sampling {num_dust} dust, {num_dirt} dirt, {num_long_hair} long hairs, "
                  f"{num_short_hair} short hairs, {num_scratch} scratches + {num_extra_scratch} extra scratches...")

        # ==================== KEY CHANGE: Use ONLY df_synthetic ====================
        df_merged = df_synthetic[['Type', 'Non-zero pixel area', 'Artifact']] \
                        .rename(columns={'Non-zero pixel area': 'Contour Area'}) \
                        .copy()

        # Optional: filter only supported types
        df_merged = df_merged[df_merged['Type'].isin(['dust', 'dirt', 'short hair', 'long hair', 'scratch'])] \
                        .reset_index(drop=True)

        df_merged = df_merged.sample(frac=1).reset_index(drop=True)
        
        df1 = df_merged[df_merged['Type'] == 'dust'].reset_index(drop=True)
        df2 = df_merged[df_merged['Type'] == 'dirt'].reset_index(drop=True)
        df3 = df_merged[df_merged['Type'] == 'short hair'].reset_index(drop=True)
        df4 = df_merged[df_merged['Type'] == 'long hair'].reset_index(drop=True)
        df5 = df_merged[df_merged['Type'] == 'scratch'].reset_index(drop=True)

        # Sample target sizes from synthetic only
        resize_targets_dust = sample_size_artifacts(df_synthetic, 'dust', num_dust)
        resize_targets_dirt = sample_size_artifacts(df_synthetic, 'dirt', num_dirt)
        resize_targets_short_hair = sample_size_artifacts(df_synthetic, 'short hair', num_short_hair)
        resize_targets_long_hair = sample_size_artifacts(df_synthetic, 'long hair', num_long_hair)
        resize_targets_scratch = sample_size_artifacts(df_synthetic, 'scratch', num_scratch)
        ...
        selected_artifacts_df = pd.concat([df1, df2, df3, df4, df5], ignore_index=True)

        artifacts_num = num_dust + num_dirt + num_short_hair + num_long_hair + num_scratch + num_extra_scratch

    else: 
        # Uniform sampling - use only synthetic
        artifacts_num = np.random.randint(min_artifacts, max_artifacts)
        selected_artifacts_df = df_synthetic.sample(artifacts_num).reset_index(drop=True)
        if verbose:
            print("Number of artifacts in generated mask: ", artifacts_num)
    ...
    for _, artifact_row in selected_artifacts_df.iterrows():
        try:
            ...
            artifact = skimage_tf.rescale(artifact, round(new_rescale_factor, 2), 
            ...
            mask_final[x1:x2, y1:y2] = np.where(artifact > mask_final[x1:x2, y1:y2], 
                                                artifact, mask_final[x1:x2, y1:y2])
            i = i + 1
    ...
    # Extra scratches block (unchanged)
    if 'add_scratches' in locals() and add_scratches > 0.5:
        ...
        for scratch_length in extra_scratches_lengths:
            try: 
                ...
                mask_final[x1:x2, y1:y2] = np.where(scratch > mask_final[x1:x2, y1:y2], 
                                                    scratch, mask_final[x1:x2, y1:y2])
                i = i + 1
